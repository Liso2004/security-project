...
class SecureAuthSystem:
    def __init__(self, db_path: str = "auth_system.db"):
        ...
        self.brute_force_protection = {
            'max_attempts': 5,
            'lockout_duration': 300,  # 5 minutes in seconds for testing
            'ip_monitoring': True
        }
        ...
        if result:
            ...
            if account_locked and lock_until:
                ...
                if datetime.now() < lock_time:
                    ...
                    conn.close()
                    return False, f"Account temporarily locked. Try again in {int(remaining_time.total_seconds())} seconds"
                else:
                    # Unlock the account
    ...
    def record_failed_attempt(self, username: str, ip_address: str = None):
        ...
        if recent_attempts >= self.brute_force_protection['max_attempts']:
            ...
            print(f"*** ACCOUNT LOCKED until {lock_until.time()} ***")
...
if __name__ == "__main__":
    ...
    # Try correct password immediately after lockout - should fail
    print("Testing immediate authentication after lockout:")
    success, message, _ = auth_system.authenticate(
        "john_doe", 
        "SecurePass123!",
        ip_address="192.168.1.100"
    )
    print(f"Immediate retry: {message}")
    print()
